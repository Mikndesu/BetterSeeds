...
public class MyChestContainer extends AbstractContainerMenu {
    private final ContainerLevelAccess containerAccess;

    // Client Constructor
    public MyChestContainer(int id, Inventory playerInv) {
        this(id, playerInv, new ItemStackHandler(27), BlockPos.ZERO);
    }

    // Server constructor
    public MyChestContainer(int id, Inventory playerInv, IItemHandler slots, BlockPos pos) {
    ...
    public boolean stillValid(Player player) {
        return stillValid(this.containerAccess, player, BlockInit.getEXAMPLE_CHEST().get());
    }

    public static MenuConstructor getServerContainer(MyChestBlockEntity chest, BlockPos pos) {
        return (id, playerInv, player) -> new MyChestContainer(id, playerInv, chest.inventory, pos);
    }
}
